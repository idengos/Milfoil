...
% % preallocation of functions
  ...
  D = 1.2; % diffusion coefficient is in cm^2/day (max is 1.9 in terms of dt and dx)
  ...
  %pulling factors
  pulling_day=90;
  stolon_percent_left=0.5; %the percentage of stolons left after pulling based on what was already there
  stolon_percent_pulled=1-stolon_percent_left;
 ...
 invaded_before = numel(find(S>10^(-5)))
   %down the middle of patch
    ...
    half_graph_W=Wnew(:,1:215);
     ...
     squares_in_pulling_area= numel(row1);
     
     squares_denuted=round(squares_in_pulling_area*stolon_percent_pulled)
     
     p=randperm(squares_in_pulling_area, squares_denuted);
   ...
   length=numel(p);
    ...
    for i=1:length
        dex=p(i);
        S(row1(dex),column1(dex))=0;
    
        i=i+1;
    end

     
%     S(:,1:215)= stolon_percent_left*S(:,1:215);
    Wnew(:,1:215)=W0;
